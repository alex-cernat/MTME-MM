...
mtme_decomp_overall <- ggplot(res_std_long, aes(Form,
                                                Variance,
                                                fill = Component,
                                                group = Component))
mtme_decomp_overall <- mtme_decomp_overall +
  geom_bar(stat = "identity") + facet_wrap(~Question) +
...
mtme_decomp_bywave <- ggplot(res_std_long, aes(wave, Variance,
                                               fill = Component,
                                               group = Component))
mtme_decomp_bywave <- mtme_decomp_bywave +
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Accent") +
  theme_minimal(base_family = "Sans") +
  xlab("Wave of the question") +
  ylab("Proportion of variance explained") +
  theme(plot.margin = unit(c(0.5, 2, 0.5, 0.5), "cm")) +
  scale_y_continuous(breaks = c(0, 12, 24, 36, 48),
                     labels = c("0", "0.25","0.5", "0.75","1"))

mtme_decomp_bywave

ggsave(filename = "./graphs/var_decomp_bywave.png", dpi = 500)


# graph by wave and form

mtme_decomp_bywave_form <- ggplot(res_std_long,
                                  aes(wave,
                                      Variance,
                                      fill = Component,
                                      group = Component))
mtme_decomp_bywave_form <- mtme_decomp_bywave_form +
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Accent") + theme_minimal(base_family = "Sans") +
  xlab("Wave and form of the question") +
  ylab("Proportion of variance explained") +
  theme(plot.margin = unit(c(0.5, 2, 0.5, 0.5), "cm")) +
  facet_grid(Question ~ Form) +
  theme(strip.text.y = element_text(angle = 0))

mtme_decomp_bywave_form
...
mean(ip789_save$g1t1_7.median, na.rm = T)
