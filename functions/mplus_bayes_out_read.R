...
res_std <- res[nm,]
res_std$variable <- gsub(".+(G.+)", "\\1", rownames(res_std))
res_std$version <- (gsub("G([0-9])T[0-9]", "\\1", res_std$variable))
res_std$question <- as.numeric(gsub("G([0-9])T([0-9])", "\\2", res_std$variable))
res_std$component <- gsub("STDYX,[ ]+(.+)[ ]+BY (G.+)", "\\1", rownames(res_std))
res_std$component[grep("STDYX,  G", res_std$component)] <- "Random"
res_std$component <- gsub("(T|P)[0-9]", "\\1", res_std$component)
res_std$component <- gsub("(T|P)[0-9]", "\\1", res_std$component)
